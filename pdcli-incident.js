#!/usr/bin/env node
const incident = require('./lib/incident.js');
...
program
    ...
    .action(function () {
        ...
        incident.list().then(function (results) {
            if (asJson) {
                console.log(results);
            }
            else {
                ...
                for (let i = 0; i < parsedBody.incidents.length; i++) {
                    let record = parsedBody.incidents[i];
                    incident.display(record);
                }
            }
        });
    });
